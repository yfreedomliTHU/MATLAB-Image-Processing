...
for i=1:size(img,1)
    for j=1:size(img,2)
        if(abs(img(i,j)>=40))%设置阈值
            if(count<=ascii_len)
              if(img(i,j)<0)
                 img_bin=dec2bin(abs(img(i,j)));%替换低位为隐藏信息(负数则取绝对值） 
              else
                 img_bin=dec2bin(img(i,j));%替换低位为隐藏信息
              end 
              img(i,j)=img(i,j)-img_bin(end)+ascii_code(count);
              count=count+1;
            end
        end
    end
...
for i=1:size(re_img,1)
    for j=1:size(re_img,2)
        if(abs(re_img(i,j)>=40))
           if(count<=ascii_len)
             if(re_img(i,j)<0)
                reimg_bin=dec2bin(abs(re_img(i,j)));%替换低位为隐藏信息(负数去绝对值）
             else
               reimg_bin=dec2bin(re_img(i,j));%替换低位为隐藏信息
             end 
             getinfo(count)= reimg_bin(end)-'0';%获取信息
             count=count+1;
           end
        end
    end
...
imwrite(hall_decode,'hall_hide3.jpg');
