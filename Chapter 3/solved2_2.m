...
for j=1:size(img,2)%只选取左上角的一小部分进行编码
        if(count<=ascii_len)
           if(img(1,j)<0)
               img_bin=dec2bin(abs(img(1,j)));%替换低位为隐藏信息(负数则取绝对值） 
            else
               img_bin=dec2bin(img(1,j));%替换低位为隐藏信息
            end 
            img(1,j)=img(1,j)-img_bin(end)+ascii_code(count);
            count=count+1;
        end
 end
[L,W]=size(hall_gray);
...
for j=1:size(re_img,2)
     if(count<=ascii_len)
        if(re_img(1,j)<0)
           reimg_bin=dec2bin(abs(re_img(1,j)));%替换低位为隐藏信息(负数去绝对值）
        else
           reimg_bin=dec2bin(re_img(1,j));%替换低位为隐藏信息
        end
...
imwrite(hall_decode,'hall_hide2_2.jpg');
