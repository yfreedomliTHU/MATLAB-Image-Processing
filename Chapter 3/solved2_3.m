...
Doc='good!'; %输入文字
data=dec2bin(double(Doc))-'0';%将需要隐藏的信息转化为二进制码流
ascii_code=reshape(data',1,numel(data));%由numel函数得到矩阵的元素个数
ascii_code(ascii_code==0)=-1;%将二进制中的0换为-1
...
%信息隐藏
count=1;
for j=1:size(img,2)%只选取左上角的一小部分进行编码
        if(count<=ascii_len)
           img_pos=find(img(:,j)~=0,1,'last');%获取每一列最后一个非0数的位置
           if(img_pos==64)
               img(img_pos,j)=ascii_code(count);%替换为隐藏信息 
            else
               img(img_pos+1,j)=ascii_code(count);%非零的下一位替换为信息位
            end 
            count=count+1;
        end
 end
...
for j=1:size(re_img,2)
     if(count<=ascii_len)
        reimg_pos=find(re_img(:,j)~=0,1,'last');%获取每一列最后一个非0数的位置
        if(re_img(reimg_pos,j)==-1)
           getinfo(count)=0;
        else
           getinfo(count)= re_img(reimg_pos,j);%获取信息
        end
        count=count+1;
     end
end
...
subplot(1,2,2);imshow(hall_decode);title('加入隐藏信息后的图');
imwrite(hall_decode,'hall_hide2_3.jpg');
